...
void stopped_process(PCB_wrapper *pcb_wrapper, pid_t p) {
    while (1) {
        if ((*pcb_wrapper).flag == 0) {
            (*pcb_wrapper).flag = 1;
            for (int i = 0; i < (*pcb_wrapper).in; i++) {
    ...
    while (1)
    {
        if ((*pcb_wrapper).flag == 0)
        {
            ...
            for (int i = 0; i < (*pcb_wrapper).in; i++)
            {
                if (((*pcb_wrapper).pcb[i]).pid == p)
                {
                    ((*pcb_wrapper).pcb[i]).status = -1; // remove by assigning invalid status
                }
            }
            (*pcb_wrapper).flag = 0;
            break;
...
void handle_sigstp(int signal) {
    if (waitingpid != 0) {
        ...
        kill(waitingpid, SIGTSTP);
        stopped_process(&shm[0], waitingpid);
        waitingpid = 0;
    }
}
